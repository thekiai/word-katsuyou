...
export const IntermediateFlashcardHome = () => {
  ...
  const { isLoading, getTodayStats, getOverallStats, resetProgress } = useIntermediateFlashcardProgress();

  if (isStudying) {
    return <IntermediateFlashcardStudy onBack={() => setIsStudying(false)} />;
  }

  if (showWordList) {
    return <IntermediateWordList onBack={() => setShowWordList(false)} />;
  }
