...
const STORAGE_KEY_PROGRESS = 'reverse-intermediate-flashcard-progress';
const STORAGE_KEY_TODAY = 'reverse-intermediate-flashcard-today-stats';
...
export function useReverseIntermediateFlashcardProgress(settings: FlashcardSettings = DEFAULT_SETTINGS) {
  ...
  useEffect(() => {
    try {
      ...
      console.error('Failed to load reverse intermediate flashcard progress:', e);
    }
    setIsLoading(false);
  }, []);
